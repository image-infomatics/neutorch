...
class SwinDecoder(nn.Module):
    ...
    def __init__(self,
        ...
        num_features = [int(embed_dim / 2 ** (i+1))
                        for i in range(self.num_layers)]
        # # no patch expanding at last layer
    ...
    def train(self, mode=True):
        """Convert the model into training mode while keep layers freezed."""
        super(SwinDecoder, self).train(mode)
        self._freeze_stages()
...
class SwinUNet(nn.Module):
    ...
    def __init__(self, in_channels: int, out_channels: int):
        ...
        self.encoder = SwinEncoder(in_chans=in_channels)
        self.decoder = SwinDecoder(out_chans=out_channels)
