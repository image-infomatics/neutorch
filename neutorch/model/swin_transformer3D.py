...
class BasicLayer(nn.Module):
    ...
    Args:
        ...
        norm_layer (nn.Module, optional): Normalization layer. Default: nn.LayerNorm
        return_residuals (True): If True, forward output is tuple (x, [r_0, r_1, ..., r_depth]])
                                 where r_n is output from Swin block. Otherwise (x, None). Default: False.
    ...
    def __init__(self,
        ...
        if upsample is not None:
            self.upsample = upsample(dim=dim*2, norm_layer=norm_layer)
    ...
    def forward(self, x, residuals=None):
        ...
        Args:
            ...
            residuals (list): list of resiudals to be added to each Swin block. len(residuals) == depth. Default: None
        """
        ...
        for i, blk in enumerate(self.blocks):
            ...
            # return residual connections if asked
            if self.return_residuals:
                residuals_out.append(x)
...
class PatchUnembed3D(nn.Module):
    ...
    Args:
        in_embed_dim (int): Dimension of embedding
        out_embed_dim (int): Number of output channels.
    """

    def __init__(self, in_embed_dim,  out_channels, patch_size=(2, 4, 4)):
        super().__init__()
...
class SwinEncoder3D(nn.Module):
    """ Swin Transformer Encoder backbone.
    ...
    Args:
        ...
        return_residuals (True): If True, forward output is tuple (x, [x_1, x_2, ..., x_len(depth)]) where 
                                 x_n is a list containing the residuals from each Swin block like: [r_1, ..., r_depth]
                                 Otherwise returns (x, None). Default: False.
    ...
    def inflate_weights(self):
        ...
        for k in relative_position_bias_table_keys:
            ...
            if nH1 != nH2:
                print(f"WARNING: Error in loading {k}, passing")
            else:
                if L1 != L2:
                    S1 = int(L1 ** 0.5)
    ...
    def forward(self, x):
        ...
        for layer in self.layers:
            ...
            (x, residuals) = layer(x.contiguous())
            if self.return_residuals:
                layer_residuals.append(residuals)
        ...
        if self.return_residuals:
            return (x, layer_residuals)

        return (x, None)

    def train(self, mode=True):
        """Convert the model into training mode while keep layers freezed."""
        super(SwinEncoder3D, self).train(mode)
        self._freeze_stages()


class SwinDecoder3D(nn.Module):
    """ Swin Transformer Decoder backbone.
        A PyTorch impl of : `Swin Transformer: Hierarchical Vision Transformer using Shifted Windows`  -
          https://arxiv.org/pdf/2103.14030
    ...
    def __init__(self,
        ...
        # embedding dimensions after all transformers have been applied
        self.out_features = int(embed_dim / 2 ** (self.num_layers-1))
    ...
    def forward(self, x, layer_residuals=None):
        """Forward function.
            x (Tensor): Input.
            layer_residuals (list): List of list of residual connections to be added at each layer.
                            residual[i][d] -> depth[i][d]. layer_residuals is a list of list of residuals
                            at each depth. Default: None.
        """
        ...
        for i, layer in enumerate(self.layers):
            residuals = None
            if layer_residuals is not None:
                residuals = layer_residuals[i]

            (x, _) = layer(x.contiguous(), residuals=residuals)
    ...
    def train(self, mode=True):
        """Convert the model into training mode while keep layers freezed."""
        super(SwinDecoder3D, self).train(mode)
        self._freeze_stages()
