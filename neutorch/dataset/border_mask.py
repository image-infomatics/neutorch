...
def create_border_mask(input_data, target, max_dist, background_label, axis=0):
    ...
    for z in range(target.shape[axis]):
        ...
        target_slice = input_data[tuple(sl)] if isinstance(
            input_data, h5py.Dataset) else np.copy(input_data[tuple(sl)])
...
def create_and_write_masked_neuron_ids(in_file, out_file, max_dist, background_label, overwrite=False):
    ...
    comment = ('' if comment is None else comment + ' ') + \
        'Border masked with max_dist=%f' % max_dist
    ...
    target = group.create_dataset(
        ds_name, shape=neuron_ids.shape, dtype=neuron_ids.dtype)
