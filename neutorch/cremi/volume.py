class Volume:
    ...
    def __getitem__(self, location):
        ...
        i = tuple([round(location[d]/self.resolution[d])
                  for d in range(len(location))])

        if min(i) >= 0:
            try:
                return self.data[i]
            except IndexError as e:
                raise IndexError("location " + str(location) +
                                 " does not lie inside volume: " + str(e))

        raise IndexError("location " + str(location) +
                         " does not lie inside volume")
